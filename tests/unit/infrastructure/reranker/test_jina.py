"""Offline tests for the JinaReranker — HTTP is stubbed with
...
def _reranker(handler, api_key="jina_secret"):
    """A JinaReranker whose lazy client is replaced by a mock transport,
    keeping the real client's headers (built the same way)."""
    reranker = JinaReranker(MODEL, api_key)
...
def test_rerank_calls_the_hosted_endpoint_with_bearer_auth():
    ...
    assert seen["url"] == "https://api.jina.ai/v1/rerank"   # fixed, not configured
