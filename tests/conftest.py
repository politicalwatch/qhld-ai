...
The only thing that must be set up for *every* test is the MongoDB connection
env — ``tipi_data`` builds its ``MongoClient`` at import time from the
``MONGO_*`` vars, so they have to be in place before the package is imported
anywhere (including the model-only unit tests). The values point at a
throwaway test database on a **fixed** host port.

``MONGO_SKIP_INDEX_INIT`` keeps the import-time index creation from connecting,
so unit tests import ``tipi_data`` offline with no MongoDB.

Port 47020 is used so this suite never fights another qhld repo for the host
port: qhld-data=47017, qhld-tasks=47018, qhld-engine=47019, qhld-ai=47020.
...
os.environ.setdefault("MONGO_PORT", "47020")
